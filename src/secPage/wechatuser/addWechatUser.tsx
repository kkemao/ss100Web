...
function AddWechatUser({
  ...
  const createWechatUser = async () => {
    try {
      const res = await BaseApi.addWechatUser({
        ...info,
  ...
  const editWechatUser = async () => {
    try {
      const res = await BaseApi.updateWechatUser(info);
      if (res.statusCode === 200) {
        refresh();
      }
      message.info(res.msg);
  ...
  const onFinish = (values: any) => {
    ...
    mode === ModeType.CREATE ? createWechatUser() : editWechatUser();
  };
      ...
      >
        ...
        >
          <Radio.Group onChange={onTypeChange} value={info.role}>
            ...
            <Radio value={roleConstans.ADMIN}>会员</Radio>
            <Radio value={roleConstans.SUPER_ADMIN}>超级会员</Radio>
          </Radio.Group>
        </Form.Item>
        <Form.Item
          label="性别"
          ...
          {info.avatar ? (
            ...
            >
              <img
                ...
                src={imageAddPrefix(info.avatar || "")}
              />
              <Button
                icon={<DeleteOutlined />}
                type="link"
                onClick={() => {
                  setInfo({ ...info, avatar: null });
                }}
              >
                删除
              </Button>
...
export const showWechatUserModal = (props: Props) => {
  const container = document.createElement("div");
  document.body.appendChild(container);
  ReactDom.render(<AddWechatUser {...props} />, container);
  return {
    destory: () => {
